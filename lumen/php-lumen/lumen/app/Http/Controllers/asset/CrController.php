...
class CrController extends Controller
{
        ...
        if ($validator->fails()) {
            ...
            if ($formAction == "add") {
                ...
                if (!empty($result_id)) {
                    ...
                    $this->prpohistoryadd(array('pr_po_id' => $result_id_text, 'history_type' => $pr_po_type, 'action' => 'created', 'details' => $hist_details, 'created_by' => DB::raw('UUID_TO_BIN("' . $request->input('loggedinuserid') . '")')));
                    //Add into UserActivityLog
            ...
            } else //if($formAction == "edit")
            {
                ...
                if ($result) {
                    ...
                    //Add into Purchase History
                    $this->prpohistoryadd(array('pr_po_id' => $pr_id_uuid, 'history_type' => $pr_po_type, 'action' => 'updated', 'details' => $hist_details, 'created_by' => DB::raw('UUID_TO_BIN("' . $request->input('loggedinuserid') . '")')));
                    //Add into UserActivityLog
        ...
        try
        {
            ...
            if ($validator->fails()) {
                ...
                if ($result) {
                    ...
                    $this->prpohistoryadd(array('pr_po_id' => $pr_po_id_uuid, 'history_type' => $request['pr_po_type'], 'action' => $request->input('approval_status'), 'details' => $hist_details, 'created_by' => DB::raw('UUID_TO_BIN("' . $request->input('loggedinuserid') . '")'), 'comment' => $comment));
                ...
                if ($result) {
                    ...
                    if ($flag == 1) {
                        ...
                        if ($action == "notifyagain") {
                            ...
                            $this->prpohistoryadd(array('pr_po_id' => $pr_po_id_uuid, 'history_type' => $history_type, 'action' => $action, 'details' => $hist_details, 'created_by' => DB::raw('UUID_TO_BIN("' . $request->input('loggedinuserid') . '")'), 'notify_to_id' => $notify_to_id, 'comment' => $comment));
                        }
        ...
        if ($validator->fails()) {
            ...
            if (empty($inv)) {
                ...
                if ($dtl) {
                    ...
                    $this->prpohistoryadd(array('pr_po_id' => $request['pr_id'], 'history_type' => 'pr', 'action' => 'Convert to PR', 'details' => $hist_details, 'created_by' => DB::raw('UUID_TO_BIN("' . $request['user_id'] . '")'), 'comment' => ''));
                    //Add into UserActivityLog
    ...
    public function assignprtouser(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if ($invoiceData) {
                ...
                $hist_details = 'pr assigned to user';
                $this->prpohistoryadd(array('pr_po_id' => $request['pr_po_id'], 'history_type' => 'pr', 'action' => 'updated', 'details' => $hist_details, 'created_by' => DB::raw('UUID_TO_BIN("' . $request['user_id'] . '")'), 'comment' => ''));
                //Add into UserActivityLog
        ...
        if ($validator->fails()) {
            ...
            if ($prpoattachment) {
                ...
                if (!unlink($attachment_name)) {
                    ...
                    //Add into Purchase History
                    $this->prpohistoryadd(array('pr_po_id' => $request->input('pr_po_id'), 'history_type' => $request['attachment_type'], 'action' => 'deleted', 'details' => $hist_details, 'created_by' => DB::raw('UUID_TO_BIN("' . $request->input('loggedinuserid') . '")')));
                    //Add into UserActivityLog
    ...
    public function quotation_comparison_approval(Request $request)
    {
        ...
        $this->prpohistoryadd(array('pr_po_id' => $request['pr_po_id'], 'history_type' => 'pr', 'action' => 'updated', 'details' => $hist_details, 'comment' => 'NA', 'created_by' => DB::raw('UUID_TO_BIN("' . $request['created_by'] . '")')));
    ...
    public function quotation_comparison_save(Request $request)
    {
        ...
        if ($res) {
            ...
            $this->prpohistoryadd(array('pr_po_id' => $request['pr_po_id'], 'history_type' => 'pr', 'action' => 'added', 'details' => $hist_details, 'comment' => 'NA', 'created_by' => DB::raw('UUID_TO_BIN("' . $request['created_by'] . '")')));
        ...
        try
        {
            ...
            foreach ($request->input('file') as $key => $filename) {
                ...
                if (file_put_contents($file_dir, $decoded_file)) {
                    ...
                    //Add into Purchase History
                    $this->prpohistoryadd(array('pr_po_id' => $request->input('pr_po_id'), 'history_type' => $request['attachment_type'], 'action' => 'open', 'details' => $hist_details, 'created_by' => DB::raw('UUID_TO_BIN("' . $request->input('loggedinuserid') . '")')));
                    //Add into UserActivityLog
            ...
            if ($validator->fails()) {
                ...
                } else //if($formAction == "edit")
                {
                    ...
                    if ($result) {
                        ...
                        //Add into Purchase History
                        $this->prpohistoryadd(array('pr_po_id' => $po_id_uuid, 'history_type' => $pr_po_type, 'action' => 'updated', 'details' => $hist_details, 'created_by' => DB::raw('UUID_TO_BIN("' . $request->input('loggedinuserid') . '")')));
                        //Add into UserActivityLog
    ...
    public function poinvoicedelete(Request $request)
    {
        try
        {
            ...
            if ($validator->fails()) {
                ...
                $this->prpohistoryadd(array('pr_po_id' => $po_id_uuid, 'history_type' => 'po', 'action' => 'deleted', 'details' => $hist_details, 'comment' => $inv_detail_id, 'created_by' => DB::raw('UUID_TO_BIN("' . $request->input('loggedinuserid') . '")')));
        ...
        if ($validator->fails()) {
            ...
            if (1) {
                ...
                if (!empty($result_id)) {
                    ...
                    //Add into Purchase History
                    $this->prpohistoryadd(array('pr_po_id' => $result_id_text, 'history_type' => $pr_po_type, 'action' => 'created', 'details' => $hist_details, 'created_by' => DB::raw('UUID_TO_BIN("73b9c9b8-87f8-11ec-afe5-86bd6599c53f")')));
                    //Add into UserActivityLog
                    userlog(array('record_id' => $result_id_text, 'data' => $request->all(), 'action' => 'added', 'message' => showmessage('104', array('{name}'), array($result_message))));
                    DB::commit();
                } else {
                ...
                if (!empty($basic_details) && !empty($item_details)) {
                    ...
                    if (!empty($invalidsku)) {
                     ...
                     <tr>
                        ...
                        $to_emails    = 'harshad.jadhav@esds.co.in';
                    ...
                    if (1) {
                        ...
                        if (!empty($result_id)) {
                            ...
                            //Add into Purchase History
                            $this->prpohistoryadd(array('pr_po_id' => $result_id_text, 'history_type' => $pr_po_type, 'action' => 'created', 'details' => 'Auto Purchase Request created.', 'comment' => 'Auto Approved', 'created_by' => DB::raw('UUID_TO_BIN("73b9c9b8-87f8-11ec-afe5-86bd6599c53f")')));
                            //Add into UserActivityLog
                            userlog(array('record_id' => $result_id_text, 'data' => $request->all(), 'action' => 'added', 'message' => showmessage('104', array('{name}'), array($result_message))));
