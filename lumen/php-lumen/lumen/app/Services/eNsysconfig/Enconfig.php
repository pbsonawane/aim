...
class Enconfig
{
    ...
    public function load_config($config_item = "", $codeigniter = true)
    {
        ...
        try {
            ...
            else if ($from == "local")
            {
                apilog("+++++LOCAL++++++++");
                apilog("++++++++++++++============");
                apilog("-".config('app.en_sysconfig_config_path'));                
                ...
                {
                    $en_sysconfig_config_path = config('app.en_sysconfig_config_path'); // As after process_config wont received "app.en_sysconfig_config_path"
                    // write file
                    $url = config('app.en_sysconfig_api_url').'/config/get?config_item='.$config_item;
                    $this->item = $restapi->apicall('get', $url);
                    $this->item = $this->process_config($this->item);
                    ...
                    $config_str = '';
                    apilog("---5".config('app.en_sysconfig_config_path')); 
                    if ($config_key_value)
                    {
                    ...
                    if (!is_writable($en_sysconfig_config_path))
                    {
                        
                        die(trans('messages.155')." => ".$en_sysconfig_config_path."/".$this->ensysconfig);
                    }
                    $write = file_put_contents($en_sysconfig_config_path."/".$this->ensysconfig, $config_str);
                    if (!$write)
                    {
    ...
    public function process_config($response)
    {
        ...
        apilog("---4".config('app.en_sysconfig_config_path')); 
        return $content;
    }
}