...
<div id="content">
  <div class="row">
    <div class="col-md-12">
      <form type="hidden" class="save_dbtbl_prm" id="save_dbtbl_prm" name="save_dbtbl_prm" method="post" action="{{ url('/Export_track_pr') }}">
        @csrf
...
<script>
  ...
  $(document).on("click", "#export_excel", function () { 
    ...
    $(document).on("click", ".add_remark", function () {
	    ...
	    $('#myModal_add_remark').modal('show');
	    $("#myModal_add_remark #pr_id").val(pr_id);
	});

  	/* Submit Add Remark */
  	$(document).on("click", "#submit_add_remark", function() {
		var url = '';
		var postData = $("#form_add_remark").serialize();
		url = SITE_URL + '/add_pr_remark';
		if (url != '') 
		{
