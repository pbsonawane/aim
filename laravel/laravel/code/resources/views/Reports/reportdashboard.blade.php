...
<div id="content">
  <div class="row">
    ...
    <div class="col-md-12">
    	<form method="post" name="frmrepcat" id="frmrepcat" onsubmit="return false;">  	
      		<div class="panel">
    				<?php echo csrf_field(); ?>	
              ...
              <div class="col-md-3" id="tree_data">
                <div class="panel">
                  <div class="panel-heading">
                    <div id="godashbord" class="input-group date panel-icon">
                      <i class="fa fa-list-alt" style="cursor:none !important;"></i>
                      ...
                      <div class="filter__list-wrap mb-0">
                        <ul class="filter__tree" style="display: block;">
                          <li class="filter__tree-item active">
                            ...
                            $report_category = isset($reportcategory['report_category']) ? $reportcategory['report_category'] : '';
                            $report_category   = str_replace(' ', '_', $report_category);
                            ?>
                            <li class="filter__tree-item">
                              <span class="filter__link <?php echo $active;?>" data-toggle="collapse" data-target="#collapse<?php echo $report_category; ?>" aria-expanded="false" aria-controls="collapse<?php echo $report_category; ?>"><i class="fa fa-folder mr-2"></i><span>  <?php echo isset($reportcategory['report_category']) ? $reportcategory['report_category'] : '' ?></span></span>
                            </li>
                            <?php
                              $i++;
                              }
                              ...
                              echo '<li class="filter__tree-item"><span class="filter__link"><i class="fa fa-folder mr-2"></i><span> '.trans('messages.msg_norecordfound').'</span></span></li>';
                            ?>
                            </ul>
                          </li>
                        </ul>
              ...
              <div class="col-md-9" id="report_data">
                <div class="panel-group" id="accordion" role="tablist" aria-multiselectable="true">
                  ...
                  @php
                    ...
                    $rep_category = isset($category['report_category']) ? $category['report_category'] : '';
                    $rep_category = str_replace(' ', '_', $rep_category);
                  @endphp
                    <div class="panel panel-area"  data-cat="{{ $rep_category }}">
                      <div class="panel-heading" role="tab" id="heading{{ $rep_category }}">
                        <h4 class="panel-title" style="padding:1%;">
                        <a role="button" data-toggle="collapse" data-parent="#accordion" href="#collapse{{ $rep_category }}" aria-expanded="false" aria-controls="collapse{{ $rep_category }}">
                          <span class="panel-icon"><i class="fa fa-list"></i></span>
                          <span class="panel-title">
                            {{ isset($category['report_category']) ? $category['report_category'] : '' }}
                            </span>   
                            </a>
                      ...
                      <div id="collapse{{ $rep_category }}" class="panel-collapse collapse {{$in}}" role="tabpanel" aria-labelledby="heading{{ $rep_category }}">
                        <div class="panel-body">
                          <table class="table table-striped table-bordered table-hover">
                            ...
                            @if($category['report_cat_id'] == $report['report_cat_id'])
                            <?php
                              $id   = $report['report_id'];
                              $edit = $delete = '';
                              if(canuser('update','reports'))
                            ...
                            <tr>
                              <?php
                                if($report['module'] == 'PURCHASE REQUEST')
                                {?> 
                                    <td><a href="<?php echo config('app.site_url'); ?>/prreports/details/<?php echo $id;?>"><?php echo $report['report_name']; ?></a></td>
                                    <td><?php echo $edit.' '.$delete; ?></td>
                                    <?php
                                }
                                else if($report['module'] == 'PURCHASE')
                                { ?>
                                  <td><a href="<?php echo config('app.site_url'); ?>/poreports/details/<?php echo $id;?>"><?php echo $report['report_name']; ?></a></td>
                                  <td><?php echo $edit.' '.$delete; ?></td>
                                <?php }
                                else
                                { ?>
...
<script language="javascript" type="text/javascript" src="<?php echo config('app.site_url'); ?>/enlight/scripts/reports/tags.js">
</script>
<script language="javascript" type="text/javascript" src="<?php echo config('app.site_url'); ?>/enlight/scripts/reports/reports.js"></script>
...
$('.filter__link').click(function(e) {
  ...
  clickLabel = clickLabel.trim().replace(/ /g,"_");
  console.log(clickLabel);
  /*$('.panel-area').each(function(index, item) 
  {
    if ($(".panel-area").data('cat') == clickLabel) 
    {
      $(".panel-area").toggle();
    }else
    {
  ...
  //$('.panel-area[data-cat="' + clickLabel + '"]').toggle();
});
</script>
<style type="text/css">
.chat {
