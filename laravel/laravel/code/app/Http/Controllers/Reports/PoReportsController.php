 ...
 * @package report
 */
class PoReportsController extends Controller
{
  /**
  ...
  */
  ...
  public function reportDetailsList(Request $request)
  {
    try
    {
      ...
      $form_params['report_id'] = $report_id;
      $options                  = ['form_params' => $form_params];
      $reports_resp = $this->itam->getreportdetails_po($options);
 ...
 */
  ...
  public function downloadPDF(Request $request) 
  {
    ...
    $response['ItemFinalUnitSku'] = $ItemFinalUnitSku;
  ...
  */
  public function getReportFormData()
  {
    ...
    switch ($field) 
    {
      case "vendor":
          ...
          if ($vendorsDetailsArr)
          {
            foreach ($vendorsDetailsArr as $vendor)
            {
              ...
              $vendorsDetailsOptions .= "<option value='".$vendor['vendor_id']."' ".$selected.">".$vendor['vendor_name']." - ".$vendor['vendor_unique_id']."</option>";
            }
          }
      ...
      case "po_status":
        $postatusOptions = '';
        if($reportids=='c52fbb74-abff-11ec-b94a-66250a963964')
        {
          $po_status = trans('commonarr.pr_status');
        }else{
