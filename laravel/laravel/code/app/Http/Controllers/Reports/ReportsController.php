...
class ReportsController extends Controller
{
  ...
  */
  ...
  /**
  * This controller function is used to load reports add form.
  * @author Shadab Khan
  * @access public
  * @package reports
  * @return string
  */
  public function reportsadd(Request $request)
  {
    ...
    if ($request->input('module') && $request->input('module')!="") 
    {
      ...
      {
        $ci_templ = $request->input('ci_templ_'.$reportmodules[0]['module_key']);
        if ($ci_templ !="") 
        {
          $ci_templ                   = explode("|",$ci_templ);
          $form_params['ci_templ_id'] = isset($ci_templ[0]) ? $ci_templ[0]:"";
          $form_params['ci_type_id']  = isset($ci_templ[1]) ? $ci_templ[1]:"";
          $options   = ['form_params' => $form_params];
          ...
          $reportsdata[0]['ci_templ_id']  = isset($ci_templ[0]) ? $ci_templ[0]:"";
          $reportsdata[0]['ci_type_id']   = isset($ci_templ[1]) ? $ci_templ[1]:"";
        }
      }
    ...
    $html                   = view("Reports/reportsadd", $data);
    echo $html;
  }
  /**
  * This controller function is used to save reports data in database.
  * @author Shadab Khan
  * @access public
  * @package reports
  * @param string $report_name report name
  * @param string UUID $report_cat_id reports category id
  * @return json
  */
  public function reportsaddsubmit(Request $request)
  {
    try
    {
      ...
      {
        for ($i=1; $i <= count($postData['filter_column']); $i++) 
        { 
          ...
          {
            $criteria_value = $inputdata['criteria_value-'.$i];
          }
          array_push($criteria_value_arr,$criteria_value);
          unset($inputdata['criteria_value-'.$i]);
        }
        if (count($criteria_value_arr)>0) 
        {
          $postData['criteria_value'] = $criteria_value_arr;
        }
      }

      unset($inputdata['filter_column']);
      unset($inputdata['criteria']);
      unset($inputdata['criteria_match']);

      $filters              = json_encode($postData);
      $inputdata['filters'] = $filters;

      unset($inputdata['ci_templ_id']);
      unset($inputdata['ci_type_id']);
    ...
    catch (\Exception $e)
    {
      ...
      save_errlog("reportsaddsubmit","This controller function is implemented to save reports.",$this->request_params,$e->getmessage());  
      echo json_encode($data, true);
    }
    catch (\Error $e)
    {
  ...
  /**
  * This controller function is used to load reports edit form with existing data for selected reports.
  * @author Shadab Khan
  * @access public
  * @package reports
  * @param \Illuminate\Http\Request $request
  * @param $report_id reports Unique Id
  * @return string
  */
  public function reportsedit(Request $request)
  {
    ...
    if (isset($data[0]['module']) && $data[0]['module']!="")
    {
      ...
      if ($reportmodules[0]['module_key'] == "CMDB" || $reportmodules[0]['module_key'] == "ALLCOMP")
      {
        ...
        if ($ci_templ_id !="" && $ci_type_id !="") 
        {
          ...
          $data[0]['ci_templ_id']  = $ci_templ_id;
          $data[0]['ci_type_id']   = $ci_type_id;
        }
      }
    ...
    $response['reportsdata']    = $data;
    $response['reportmodules']  = $reportmodules[0];
    $response['reportcategory'] = $reportcategory;
    $response['view']           = "secondary";
    $html                       = view("Reports/reportsadd", $response);
    echo $html;
  }
  /**
  * This controller function is used to update reports data in database.
  * @author Shadab Khan
  * @access public
  * @package reports
  * @param UUID $report_id reports  Unique Id
  * @param string $report_category reports
  * @param string $description reports Description
  * @return json
  */
  public function reportseditsubmit(Request $request)
  {
    try
    {
      $inputdata                  = $request->all();
  ...
  /**
  * This controller function is used to delete reports data from database.
  * @author Shadab Khan
  * @access public
  * @package reports
  * @param UUID $report_id reports Unique Id
  ...
  */
  ...
  public function reportDetailsList(Request $request)
  {
    try
    {
      ...
      $form_params['report_id'] = $report_id;
      $options                  = ['form_params' => $form_params];
      $reports_resp = $this->itam->getreportdetails($options);
      /*print_r($reports_resp);
      exit;*/
 ...
 */
  ...
  */
  ...
  public function pbireports() {
        $data['emgridtop'] = $this->emlib->emgridtop('', '', '');
        $data['pageTitle'] = "Power BI Reports";
        $data['includeView'] = view("Reports/pwbi_reports", $data);
  ...
  public function submit_pbireports(Request $request) {
    ...
    if(!empty($post_data)) {
      $form_params['fromdate']    = isset($post_data['fromdate'])    ?  trim($post_data['fromdate'])    : '';
      $form_params['todate']      = isset($post_data['todate'])      ?  trim($post_data['todate'])      : '';
      $form_params['report_type'] = isset($post_data['report_type']) ?  trim($post_data['report_type']) : '';

      if(trim($post_data['report_type']) != 'open_po') {
        //validations
        if($form_params['fromdate'] == '') {
          $err_count = 1;
          $error[] = "From date must be filled out"; 
        } else {
          if(!empty($form_params['todate'])) {
            if($form_params['fromdate'] > $form_params['todate']) {
              $err_count = 1;
              $error[] = "From Date is not gretter than To Date"; 
            }
          }
        }
        if($form_params['todate'] == '') {
          $err_count  = 1;
          $error[]    = "To date must be filled out";
        }
        if($form_params['report_type'] == '') {
          $err_count  = 1;
          $error[]    = "Report Date must be filled out";
        }
      } else {
        if($form_params['todate'] == '') {
          $err_count  = 1;
      ...
      if($err_count) {
        ...
        $read_report  = $this->itam->get_pbireports(['form_params' => $form_params]);
        $datas = $read_report["content"];
        $report_type_name  = $post_data['report_type']; 
        if(count($datas) > 0) {
