...
class CrController extends Controller
{
    ...
    {
        $topfilter           = array('gridsearch' => true, 'gridadvsearch' => true, 'jsfunction' => 'crList() , crDetailsLoad()');
    ...
    {
        $topfilter = array('gridsearch' => true, 'jsfunction' => 'trackcrList()', 'gridadvsearch' => false);
            ...
            if (isset($pos_resp) && !empty($pos_resp)) {
                if($pos_resp['content']['totalrecords'] > 0) {
                    foreach ($pos_resp['content']['records'] as $key => $value) {
                        ...
                        $hod_id = $value['hod_id'];
                        // 
                        $user_options_optional = ['form_params' => array('user_id' => $user_id)];
                        ...
                        if (!(is_array($user_response_data) && count($user_response_data) > 0)) {
                            $user_response_data = array();
                            $user_response_data[0] = array();                                
                        }
                        $pos_resp['content']['records'][$key]['user_details'] = $user_response_data[0];
                        // 
                        // 
                        $requester_options_optional = ['form_params' => array('user_id' => $requester_id)];
                        ...
                        if (!(is_array($requester_response_data) && count($requester_response_data) > 0)) {
                            $requester_response_data = array();
                            $requester_response_data[0] = array();                                
                        }
                        $pos_resp['content']['records'][$key]['requester_details'] = $requester_response_data[0];
                        // 
                        // 
                        $hod_options_optional = ['form_params' => array('user_id' => $hod_id)];
                        ...
                        if (!(is_array($hod_response_data) && count($hod_response_data) > 0)) {
                            $hod_response_data = array();
                            $hod_response_data[0] = array();                                
                        }
    ...
    public function approve_reject_cr(Request $request)
    {
        try
        {
            ...
            $data = $this->itam->approve_reject_cr(array('form_params' => $postData));
        } catch (\Exception $e) {
            $data["content"] = "";
            $data["is_error"] = "";
    ...
    public function complaintraiseddetail(Request $request)
    {
        ...
        try
        {
            ...
            if ($pr_po_id != "") {
                ...
                $optionss = ['form_params' => array('user_id' => showuserid())];            
    ...
    public function complaintraisedadd()
    {
        ...
        try {
            ...
            $inputdata = array('template_name' => 'complaintraised');
            // $data      = $this->itam->getFormTemplateDefaulteConfigbyTemplateName(array('form_params' => $inputdata));
    ...
    public function complaintitremarksave(Request $request)
    {
        ...
        $data = $this->itam->addcritremark(array('form_params' => $request1));
        echo json_encode($data, true);
    }
    ...
    public function complaintstoreremarksave(Request $request)
    {
        ...
        $data = $this->itam->addcrstoreremark(array('form_params' => $request1));
        echo json_encode($data, true);
    }
